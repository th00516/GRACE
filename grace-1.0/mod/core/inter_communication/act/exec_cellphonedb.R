core.inter_communication.act.exec_cellphonedb <- function(dbpath, data.id, D, C_type) {
  ...
  withProgress({
    ...
    system(paste('cellphonedb method statistical_analysis',
    ...
    if (length(unique(chk$V2)) > 1) {
      ...
      system(paste('cellphonedb plot heatmap_plot',
                   file.path(cellphonedb_path.1, 'meta.txt'),
