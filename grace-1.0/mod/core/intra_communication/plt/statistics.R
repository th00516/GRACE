core.intra_communication.plt.statistics.bubble.1 <- function(dbpath, grp.id, G_dir, subcluster.1, subcluster.2, disp) {
  
  if (is.null(disp)) {return(NULL)}
  if (is.null(subcluster.1)) {return(NULL)}
  if (is.null(subcluster.2)) {return(NULL)}
  ...
  subcluster <- paste(subcluster.1, subcluster.2, sep = '|')
  ...
  if (is.null(dat[[subcluster]])) {return(NULL)}
  
  dt <- dat[dat$gene_a != '' & dat$gene_b != '' & dat$receptor_a != dat$receptor_b, c('gene_a', 'gene_b', 'receptor_a', subcluster)]
  dt.p <- dat.p[dat.p$gene_a != '' & dat.p$gene_b != '' & dat$receptor_a != dat$receptor_b, c('gene_a', 'gene_b', 'receptor_a', subcluster)]
  ...
  if (file.exists(cellphonedb_path.0)) {
    ...
    dt.0 <- dat.0[dat.0$gene_a != '' & dat.0$gene_b != '' & dat.0$receptor_a != dat.0$receptor_b, c('gene_a', 'gene_b', 'receptor_a', subcluster)]
...
core.intra_communication.plt.statistics.bubble.2 <- function(dbpath, grp.id, G_dir, pair, disp) {
  ...
  colnames(dt) <- c('Subcluster_with_Ligand', 'Subcluster_with_Receptor', 'Means', 'p')
  ...
  fig <- plot_ly(data = dt,
                 x = ~Subcluster_with_Ligand,
                 y = ~Subcluster_with_Receptor,
                 color = ~Means,
                 colors = 'Reds',
                 type = 'scatter', mode = 'markers',
                 marker = list(size = ~(-log10((p + 0.000001) / max(p + 0.000001)) * 3.5),
                 ...
                 hovertext = ~paste('</br> Group: ', if (G_dir == 'g1') {'Group 1'} else {'Group 2'},
                                    '</br> Mean: ', sprintf('%.4f', Means),
                                    '</br> P-value: ', sprintf('%.4f', p),
                                    '</br> Subcluster with Ligand: ', sprintf('%s', Subcluster_with_Ligand),
                                    '</br> Subcluster with Receptor: ', sprintf('%s', Subcluster_with_Receptor)))
