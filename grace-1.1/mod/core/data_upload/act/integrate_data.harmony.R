...
core.data_upload.act.integrate_data <- function(dbpath, proj.id, data.ids) {
  ...
  if (length(data.ids) > 1) {
    ...
    # withProgress({
      ...
      obj <- RunHarmony(obj, group.by.vars = 'Ident')
      ...
      obj <- RunTSNE(obj, reduction = 'harmony', dims = 1:30)
      obj <- RunUMAP(obj, reduction = 'harmony', dims = 1:30)
      
      obj <- FindNeighbors(obj, reduction = 'harmony', dims = 1:30, annoy.metric = 'cosine',
      ...
      proj_id <- proj.id
      data_id <- paste0('D', id)
      name_ <- 'Integrated.Harmony'
      desc_ <- paste(data.ids, collapse = ' + ')
      created_date <- format(Sys.time(), '%y-%m-%d')
      group_ <- paste(unique(obj$Group), collapse = ' + ')
      lib_pltf <- 'Integrated.Harmony'
      lib_type <- 'Integrated.Harmony'
      seq_pltf <- 'Integrated.Harmony'
      cell_count <- D$n_vars
      fea_count <- D$n_obs
