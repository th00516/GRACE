...
core.data_upload.act.integrate_data <- function(dbpath, proj.id, data.ids) {
  ...
  if (length(data.ids) > 1) {
    ...
    # withProgress({
      ...
      obj.list <- lapply(path.list, function(x) {
        
        D <- anndata::read_h5ad(x)
        D <- D[, D$var$isUsed]
      ...
      anchors  <- FindIntegrationAnchors(object.list = obj.list, anchor.features = features, normalization.method = 'SCT', reduction = 'rpca')
