...
core.data_upload.act.integrate_data <- function(dbpath, proj.id, data.ids) {
  ...
  if (length(data.ids) > 1) {
    ...
    # withProgress({
      ...
      obj[['scanvi']] <- CreateDimReducObject(embeddings = latent, key = 'scanvi_', assay = DefaultAssay(obj))
      ...
      obj <- RunTSNE(obj, reduction = 'scanvi', dims = 1:10)
      obj <- RunUMAP(obj, reduction = 'scanvi', dims = 1:10)
      
      obj <- FindNeighbors(obj, reduction = 'scanvi', dims = 1:10, annoy.metric = 'cosine',
      ...
      proj_id <- proj.id
      data_id <- paste0('D', id)
      name_ <- 'Integrated.scANVI'
      desc_ <- paste(data.ids, collapse = ' + ')
      created_date <- format(Sys.time(), '%y-%m-%d')
      group_ <- paste(unique(obj$Group), collapse = ' + ')
      lib_pltf <- 'Integrated.scANVI'
      lib_type <- 'Integrated.scANVI'
      seq_pltf <- 'Integrated.scANVI'
      cell_count <- D$n_vars
      fea_count <- D$n_obs
