...
core.data_upload.act.integrate_data <- function(dbpath, proj.id, data.ids) {
  ...
  if (length(data.ids) > 1) {
    ...
    # withProgress({
      ...
      obj@assays$integrated <- obj@assays$RNA
      ...
      scvi$model$SCVI$setup_anndata(adata, batch_key = 'Ident')
      ...
      obj[['scvi']] <- CreateDimReducObject(embeddings = latent, key = 'scvi_', assay = DefaultAssay(obj))
      ...
      obj <- RunTSNE(obj, reduction = 'scvi', dims = 1:10)
      obj <- RunUMAP(obj, reduction = 'scvi', dims = 1:10)
      
      obj <- FindNeighbors(obj, reduction = 'scvi', dims = 1:10, annoy.metric = 'cosine',
