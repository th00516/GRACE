core.denovo_annotation.act.select_group <- function(dbpath, proj_id, data_id, D, C_type, C_idx, switch) {
  ...
  withProgress({
    ...
    dat$var$clusters.ann.SingleR.main <- group_type
    dat$var$clusters.ann.SingleR.fine <- group_type
    
    levels(dat$var$clusters.ann.SingleR.main) <- cell_type.main
    levels(dat$var$clusters.ann.SingleR.fine) <- cell_type.fine
    ...
    dat$obs$isVariableFeature <- dat$obs_names %in% VariableFeatures(obj)
